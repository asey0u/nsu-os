...
void *counter(void *arg) {
  ...
  while (1) {
    pthread_spin_lock(&st->spin);
    Node *curr = st->first;
    if (!curr) { pthread_spin_unlock(&st->spin); return NULL; }
    pthread_spin_lock(&curr->spin);
    pthread_spin_unlock(&st->spin);
...
void *swapper(void *arg) {
  ...
  while (1) {
    pthread_spin_lock(&st->spin);
    pthread_spin_lock(&st->first->spin);
    Node* prev = st->first;
    pthread_spin_unlock(&st->spin);
    Node* curr = NULL;
    Node* next = NULL;
    ...
    while (prev->next != NULL) {
      pthread_spin_lock(&prev->next->spin);
      curr = prev->next;
      if (curr->next != NULL) {
        pthread_spin_lock(&curr->next->spin);
        next = curr->next;
        if (rand() % 3 == 0) { 
          prev->next = next;
          curr->next = next->next;
