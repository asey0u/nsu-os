...
void *counter(void *arg) {
  ...
  while (1) {
    my_spinlock_lock(&st->spin);
    Node *curr = st->first;
    if (!curr) { my_spinlock_unlock(&st->spin); return NULL; }
    my_spinlock_lock(&curr->spin);
    my_spinlock_unlock(&st->spin);
...
void *swapper(void *arg) {
  ...
  while (1) {
    my_spinlock_lock(&st->spin);
    my_spinlock_lock(&st->first->spin);
    Node* prev = st->first;
    my_spinlock_unlock(&st->spin);
    Node* curr = NULL;
    Node* next = NULL;
    ...
    while (prev->next != NULL) {
      my_spinlock_lock(&prev->next->spin);
      curr = prev->next;
      if (curr->next != NULL) {
        my_spinlock_lock(&curr->next->spin);
        next = curr->next;
        if (rand() % 3 == 0) { 
          prev->next = next;
          curr->next = next->next;
        ...
        my_spinlock_unlock(&next->spin);
      }
      my_spinlock_unlock(&prev->spin);
      prev = curr;
    }
