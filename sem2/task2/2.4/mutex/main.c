...
void *counter(void *arg) {
  ...
  while (1) {
    my_mutex_lock(&st->mutex);
    Node *curr = st->first;
    if (!curr) { my_mutex_unlock(&st->mutex); return NULL; }
    my_mutex_lock(&curr->mutex);
    my_mutex_unlock(&st->mutex);
...
void *swapper(void *arg) {
  ...
  while (1) {
    ...
    while (prev->next != NULL) {
      my_mutex_lock(&prev->next->mutex);
      curr = prev->next;
      if (curr->next != NULL) {
        my_mutex_lock(&curr->next->mutex);
        next = curr->next;
        if (rand() % 3 == 0) { 
          prev->next = next;
          curr->next = next->next;
        ...
        my_mutex_unlock(&next->mutex);
      }
      my_mutex_unlock(&prev->mutex);
      prev = curr;
    }
